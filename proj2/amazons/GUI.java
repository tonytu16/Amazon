...
class GUI extends TopLevel implements View, Reporter {
    ...
    GUI(String title) {
        ...
        addMenuButton("Settings->Make player Auto", this::makeAuto);
        addMenuButton("Settings->Make player Manual", this::makeManual);
    ...
    /**a player's path. */
    private static final Pattern PLAYER_PATH = Pattern.compile("White|black");

    /**make auto function defined above. */
    private void makeAuto(String dummy) {
        String response =
                getTextInput("Enter player \"white\" or \"black\".",
                        "Player", "plain", "");
        if (response != null) {
            Matcher mat = PLAYER_PATH.matcher(response);
            if (mat.matches()) {
                _pendingCommands.offer("auto" + mat.group());
            } else {
                String msg = "Enter a player \"white\" or \"black\"";
                showMessage(msg, "Error", "error");
            }
        }
    }

    /**make manual function defined above. */
    private void makeManual(String dummy) {
        ...
        if (response != null) {
            Matcher mat = PLAYER_PATH.matcher(response);
            if (mat.matches()) {
                _pendingCommands.offer("manual" + mat.group());
            } else {
                String msg = "Enter a player \"white\" or \"black\"";
                showMessage(msg, "Error", "error");
            }
